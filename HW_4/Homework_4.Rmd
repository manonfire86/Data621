...
In this assignment we will explore, analyze, and build a multiple linear regression and binary logistic model based on auto insurance data. The models will predict the probability that a person will both crash their car and the amount of money it would cost in the event of that person crashing the car.
...
We first download the "training" and "evaluation" data sets from GitHub:
...
To gain an understanding of relevant correlations we construct a function that filters our variables for correlations of interest.
...
We then analyze the variance inflation factors for two saturated models, one linear and on logistic.

#Variance Inflation of Linear Regression Model

```{r}
vif(lm(TARGET_AMT ~., filter(training, TARGET_FLAG==1) %>% select(!TARGET_FLAG) ))
```

#Variance Inflation of Binary Logistic Regression Model
```{r}
...
We then construct our training sets for our models below.
...
We then construct a training data set inclusive of our desired transformations.
...
We partition our data to train our models and test the accuracy and performance of our constructed models.
...
Model 2 we use the step function to apply backward elimination.


```{r}
glm_mod1 = glm(TARGET_FLAG ~., data = glm_additional_train, family = binomial(link='logit'))
```
...
```{r,eval=F}
fit_1 = step(glm_mod1,direction="both",trace=FALSE)
...
```{r,eval=F}
fit_2 <- step(glm_mod2,direction="both",trace=FALSE)
```
...
Now that the binary logistic regression model is constructed we can proceed to 
the linear regression model not using any transformations.
...
We then use STEPAIC function from the MASS package to find the best model.
...
|    Model two is no longer skewed, and while the R-Squared is small, is now above one percent accuracy. Far better than the first model 
...
Using model two for both our binary logistic model and our
our linear regression model, we update our evaluation data set 
with the final required predictions.
...
The underlying nature of this data set had a few subtle complexities. In the way
of modifications, there was a need to use regular expressions and re coded factors
in order to make the data more interpretable to our models. In addition, there
was a large focus on transforming our variables to smooth out the distributions
and reduce skewness. After processing the data and transforming the necessary
variables, we were able to determine that the second iteration of our models
performed the best. It most accurately interpreted the data and seemed best
poised to deal data abstractions.