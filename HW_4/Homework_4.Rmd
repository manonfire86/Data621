...
training <- training %>% mutate(URBAN_DRIVER = factor(ifelse(URBANICITY == "Highly Urban/ Urban", 1, 0))) %>% select(-URBANICITY)

evaluation <- evaluation %>% mutate(URBAN_DRIVER = factor(ifelse(URBANICITY == "Highly Urban/ Urban", 1, 0))) %>% select(-URBANICITY)
...
We need to partition the training data to begin model construction
...
First we will construct the our initial Binary model as this will be the initial run in order for our linear model to run optimally.


```{r}
mod_fit1 = glm(TARGET_FLAG ~., data = glm_additional_train, family = binomial(link='logit'))

```


Running a summary analysis of the model we can get a quick view of the corresponding p-values and begin to discern
which variables should be pulled out for our second model.
...
Initial glance indicates that there are a number of irrelevant variables in this model. This can be due to lack of
data transformations to address the skewed/nonlinear variables. In order to test we apply boxcox transformations to discern whether this is in fact the case.
...
Reapplying the training partition we can now review the model with transformed variables.

```{r}
lm_train_index <- createDataPartition(lm_training_transform$TARGET_AMT, p=.8, list=FALSE, times = 1)
lm_additional_train <- lm_training[lm_train_index,]
lm_additional_test <- lm_training[-lm_train_index,]


glm_train_index <- createDataPartition(glm_training_transform$TARGET_FLAG, p=.8, list=FALSE, times = 1)
glm_additional_train <- glm_training[glm_train_index,]
glm_additional_test <- glm_training[-glm_train_index,]
```
...
mod_fit1_trans = glm(TARGET_FLAG ~., data = glm_additional_train, family = binomial(link='logit'))
summary(mod_fit1_trans)
```

Post transformation the variable application has become statistically significant and we can begin to use a process of backward elimination for variable selection.

Model 2 I use the step function to apply backward elimination.


```{r}
mod_fit2_trans = step(mod_fit1_trans,direction="backward",trace=FALSE)
summary(mod_fit2_trans)
```


Model 3 I use the step function to apply backward elimination for a final model.

```{r}
mod_fit3_trans = step(mod_fit2_trans,direction="backward",trace=FALSE)
summary(mod_fit3_trans)
```

# Model Selection

Below are model comparison analytics showing which models best fit the underlying data.

```{r model_perf_analy,eval=FALSE}
# anova analysis
anova(mod_fit1_trans,mod_fit2_trans,mod_fit3_trans)
# Goodness of fit tests
anova(mod_fit1_trans,mod_fit2_trans,mod_fit3_trans, test ="Chisq")
## Likelihood Ratio
lrtest(mod_fit1_trans,mod_fit2_trans,mod_fit3_trans)
...
HLgof.test(fit = fitted(mod_fit1_trans), obs = glm_additional_train$TARGET_FLAG)
HLgof.test(fit = fitted(mod_fit2_trans), obs = glm_additional_train$TARGET_FLAG)
HLgof.test(fit = fitted(mod_fit3_trans), obs = glm_additional_train$TARGET_FLAG)


```

We then predict results to discern performance metrics.

```{r}
results_model_one = predict.glm(mod_fit1_trans,newdata = glm_additional_test)
results_model_two = predict(mod_fit2_trans,newdata = glm_additional_test)
results_model_three = predict(mod_fit3_trans,newdata = glm_additional_test)
...
```{r}
confusionMatrix(data=results_model_one, glm_additional_test$TARGET_FLAG)
confusionMatrix(data=results_model_two, glm_additional_test$TARGET_FLAG)
confusionMatrix(data=results_model_three, glm_additional_test$TARGET_FLAG)
```


```{r}
glm_prob_one <- predict(mod_fit1_trans, newdata=glm_additional_test, type="response")
...
str(glm_additional_test)
```


```{r}
glm_prob_two <- predict(mod_fit2_trans, newdata=glm_additional_test, type="response")
glm_pred_two <- prediction(glm_prob_two, glm_additional_test$TARGET_FLAG)
...
```{r}
glm_prob_three <- predict(mod_fit3_trans, newdata=glm_additional_test, type="response")
glm_pred_three <- prediction(glm_prob_three, glm_additional_test$TARGET_FLAG)
glm_perf_three <- performance(glm_pred_three, measure = "tpr", x.measure = "fpr")
plot(glm_perf_three)
glm_auc_mod3 <- performance(glm_pred_three, measure = "auc")
glm_auc_mod3 <- glm_auc_mod3@y.values[[1]]
